...
func init() {
	...
	// instead of matching an existing one. See design-notes/13 for motivation.
...
func getAuthInfo(settings *config.Settings, endpoint string) authInfo {
	...
	// Exception: inside an agent container this token IS the identity we're
	// authenticating as, so it must not be shadowed by dev auth — see
	// isInAgentContext.
	if token := readAgentTokenFile(); token != "" {
		...
		if isLocalhostEndpoint(endpoint) && !isInAgentContext() {
...
func getHubClient(settings *config.Settings) (hubclient.Client, error) {
	...
	if !authConfigured {
		if token := readAgentTokenFile(); token != "" {
			if !apiclient.IsDevToken(token) && isLocalhostEndpoint(endpoint) && !isInAgentContext() {
...
func runHubLink(cmd *cobra.Command, args []string) error {
	...
	// existing project. See design-notes/13 for design.
	if hubLinkNew {
...
// name catches the wrong existing project. See design-notes/13.
