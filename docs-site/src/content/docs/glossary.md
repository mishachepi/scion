...
### Runtime
The technology that executes an agent. Container backends: Docker, Podman, Apple Container, Kubernetes, or Cloud Run Instances; the **tmux** backend runs agents as host processes inside tmux windows with no container — see [The tmux Runtime](/scion/local/tmux-runtime/).
