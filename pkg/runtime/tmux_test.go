...
func TestBuildGitCloneArgs(t *testing.T) {
	cases := []struct {
		...
		token     string
		wantArgs  []string
		wantURLIn string // substring expected in the URL position (args[5])
	}{
		{
		...
		},
		{
			...
			gc:        api.GitCloneConfig{URL: "https://github.com/example/repo.git", Branch: "feat/x", Depth: 5},
			token:     "",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(got) != 7 {
				t.Fatalf("expected 7 args, got %d: %v", len(got), got)
			}
			if got[5] != tc.wantURLIn {
				t.Errorf("URL arg = %q, want %q", got[5], tc.wantURLIn)
			}
