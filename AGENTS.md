...
- **Agent-token expiry self-heals via the token file (since abf8d051).** When the hub rejects a token refresh (401/403), sciontool's refresh loop re-reads `~/.scion/scion-token` and adopts a different still-valid token (`adoptTokenFromFile` in `pkg/sciontool/hub/client.go`). Fleet recovery from expired tokens = rewrite the token files (e.g. operator's `scion-refresh-agent-tokens.py`); agents recover within one retry (≤5min backoff), no per-process SIGUSR2 needed. Caveat: that operator script re-derives the hub signing key by duplicating `pkg/hub/server.go::deriveSharedSigningKey` — if the derivation or token format changes, the script silently breaks.
- **`scion start` vs `scion resume` for existing agents.** `start` on an existing stopped agent returns 409 (it goes through the create path); `scion resume <agent>` is the restart command — for a *suspended* agent it continues the harness session (`--resume <session-id>`), for a *stopped* one it restarts in place with a fresh session (stop is terminal). Consequence: **suspend → resume is the zero-context-loss binary upgrade path** — the sciontool/harness processes are recreated from the on-disk binaries while the session continues (used for the 2026-07-26 fleet rollout).
- **Auto-link silently overwrites `project_contributors.local_path` (footgun, fix candidate).** A hub-mode CLI invocation with `--project <slug>` may re-link the project with a path derived from the caller's cwd, replacing a working `local_path` without validation. Incident 2026-07-26: a `~/demo` directory (name matching the project slug, containing a stale `.scion` skeleton with an outdated `project-id`) poisoned the workstation contributor row → broker-side resume failed with "agent does not exist" (`pkg/agent/run.go` existence check against the wrong agents dir). Recovery: run any hub CLI command from the real project directory — auto-link then rewrites the path correctly. Proper fix: don't overwrite a non-empty `local_path` (or at least require the new path's `agents/` dir to be non-empty / `project-id` to match).
- **Host-side test failures that are NOT CI signal (macOS operator machine).** Three pre-existing local-only failure classes in `make ci`: (1) `pkg/config`/`pkg/agent` path tests fail on the `/var` → `/private/var` symlink (t.TempDir canonicalization); (2) tests sensitive to leaked operator env (`OPENAI_API_KEY` present → `TestStartBrokerMode_EmptyEnvNotFatal` fails); (3) `pkg/runtimebroker` `TestCreateAgent*` read the operator's real `~/.scion` broker settings (`allow_container_script_harnesses=false` → 403s). Verify a suspicious failure against clean HEAD before attributing it to your change; fixing these tests to isolate properly is welcome adjacent cleanup.
